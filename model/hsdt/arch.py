...
class HSDT(nn.Module):
    def __init__(self, in_channels:int, channels:int, num_half_layer:int, sample_idx:list, Fusion=None):
        ...
        Args:
            in_channels (int): HSI的通道数，这里非band数目
            channels (int): HSI第一层输出的通道数，非band数目
            num_half_layer (int): 对称的encoder或decoder数目
            sample_idx (list): 进行上下采样的位置指标
            Fusion (_type_, optional): encoder与decoder阶段输出的融合. Defaults to None.
        """
...
if __name__ == '__main__':
    ...
    model = HSDT(in_channels=1, channels=32, num_half_layer=2, sample_idx=[0,1])
